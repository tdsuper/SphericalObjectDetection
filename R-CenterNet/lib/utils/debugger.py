...
class Debugger(object):
    def __init__(self, ipynb=False, theme='black',
        ...
        if dataset == 'coco_hp':
            ...
            self.colors_hp = [(255, 0, 255), (255, 0, 0), (0, 0, 255),
                              ...
                              (255, 0, 0), (0, 0, 255), (255, 0, 0), (0, 0, 255),
                              (255, 0, 0), (0, 0, 255)]
        elif num_classes == 48 or dataset == 'coco':
    ...
    def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default'):
        # bbox[:2] = np.array(bbox[:2], dtype=np.int32)
        bbox[2] = bbox[2] * 180 / np.pi
        bbox[3] = bbox[3] * 180 / np.pi
        # print(bbox)
        # cat = (int(cat) + 1) % 80
        cat = int(cat)
        # print(cat)
        ...
        img_size = self.imgs[img_id].shape
        theta = (bbox[0] - 0.5) * np.pi * 2 / img_size[1] - np.pi
        phi = np.pi / 2 - (bbox[1] - 0.5) * np.pi / img_size[0]
        angle = bbox[4] / np.pi * 180
        # print(angle)
        # print(theta, phi, bbox[2], bbox[3])
        ra = 10
        BFoV = ImageRecorder(img_size[1] // ra, img_size[0] // ra, view_angle_w=bbox[2], view_angle_h=bbox[3],
                             long_side=img_size[1] // ra)
        Px, Py = BFoV._sample_points(theta, phi, border_only=True)
        Px, Py = ro_Shpbbox(theta, phi, Px, Py, angle, erp_w=img_size[1] // ra, erp_h=img_size[0] // ra)
        for j in range(Px.shape[0]):
            cv2.circle(self.imgs[img_id], (int(Px[j]*ra), int(Py[j]*ra)), 2, c)

        if show_txt:
            cv2.rectangle(self.imgs[img_id],
                          (int(Px[0]*ra), int(Py[1]*ra - cat_size[1] - 2)),
                          (int(Px[0]*ra + cat_size[0]), int(Py[1]*ra - 2)), c, -1)
            cv2.putText(self.imgs[img_id], txt, (int(Px[0]*ra), int(Py[1]*ra - 2)),
...
coco_class_name = [
        'airconditioner', 'backpack', 'bathtub', 'bed', 'board', 'book', 'bottle', 'bowl', 'bucket', 'cabinet',
         'chair', 'clock', 'clothes', 'computer', 'cup', 'cushion', 'door', 'extinguisher', 'fan', 'faucet',
         'fireplace', 'fruits', 'heater', 'keyboard', 'light', 'microwave', 'mirror', 'mouse', 'outlet', 'oven',
         'paper extraction', 'person', 'phone', 'picture', 'potted plant', 'refrigerator', 'shoes', 'sink', 'sofa',
         'table', 'toilet', 'towel', 'tv', 'vase', 'washer', 'water dispenser', 'window', 'wine glass']
    # self._valid_ids = [

pascal_class_name = [
    'toilet', 'board', 'mirror', 'bed', 'potted plant', 'book',
    'clock', 'phone', 'keyboard', 'tv', 'fan', 'backpack',
    'light', 'refrigerator', 'bathtub', 'wine glass', 'airconditioner', 'cabinet',
    'sofa', 'bowl', 'sink', 'computer', 'cup', 'bottle',
    'washer', 'chair', 'picture', 'window', 'door', 'heater',
    'fireplace', 'mouse', 'oven', 'microwave', 'person', 'vase', 'table'
]
