...
def xy2XYZ(x, y, output_w, output_h):
    theta, phi = (x - 0.5) / output_w * 2 * np.pi, (y - 0.5) / output_h * np.pi
    X = torch.sin(phi) * torch.cos(theta)
    Y = torch.sin(phi) * torch.sin(theta)
    Z = torch.cos(phi)
    return X, Y, Z

def XYZ2xy(X, Y, Z, output_w, output_h):
    phi = torch.acos(Z)
    theta = torch.atan2(Y, X)
    theta[theta < 0] += 2 * np.pi
    x = theta * output_w / (2 * np.pi) + 0.5
    y = phi * output_h / np.pi + 0.5
    return x, y
...
def ctdet_decode(heat, wh, angle, reg=None, cat_spec_wh=False, K=100):
    ...
    xs = xs.view(batch, K, 1)
    ys = ys.view(batch, K, 1)
    X, Y, Z = xy2XYZ(xs, ys, width, height)
    # print(xs, ys)
    if reg is not None:
      reg = _transpose_and_gather_feat(reg, inds)
      ...
      X = X.view(batch, K, 1) + 0.5
      Y = Y.view(batch, K, 1) + 0.5
      Z = Z.view(batch, K, 1) + 0.5
    norm = torch.sqrt(X ** 2 + Y ** 2 + Z ** 2)
    X, Y, Z = X / norm, Y / norm, Z / norm

    xs, ys = XYZ2xy(X, Y, Z, width, height)
    # print(xs, ys)
    wh = _transpose_and_gather_feat(wh, inds)
    angle = _transpose_and_gather_feat(angle, inds)
    if cat_spec_wh:
      wh = wh.view(batch, K, cat, 2)
      angle = angle.view(batch, K, cat, 1)
      clses_ind = clses.view(batch, K, 1, 1).expand(batch, K, 1, 2).long()
      wh = wh.gather(2, clses_ind).view(batch, K, 2)
      angle = angle.gather(1, clses_ind).view(batch, K, 1)
    else:
      wh = wh.view(batch, K, 2)
